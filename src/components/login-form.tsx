...
export function LoginForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Navigation handled by context or we can force it here just in case
      // navigate("/priming")
    } catch (err: any) {
  ...
  return (
    <form className={cn("flex flex-col gap-6 bg-card p-8 rounded-xl border shadow-sm", className)} {...props} onSubmit={handleSubmit}>
      <div className="flex flex-col items-center gap-2 text-center">
        <h1 className="text-2xl font-bold">Welcome back</h1>
        <p className="text-balance text-sm text-muted-foreground">
          Login to your KORAVO account
        </p>
      </div>
